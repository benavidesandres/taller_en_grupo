...
.stApp::before {
    ...
    inset: 0;
    background-image:
        radial-gradient(ellipse 80% 50% at 20% 20%, rgba(0,144,200,0.08) 0%, transparent 60%),
        radial-gradient(ellipse 60% 40% at 80% 80%, rgba(10,79,140,0.07) 0%, transparent 60%),
        url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%230090C8' fill-opacity='0.018'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
    pointer-events: none;
    z-index: 0;
}
...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #050e18 0%, #06101a 100%) !important;
...
[data-testid="metric-container"] {
    ...
    box-shadow: 0 4px 20px rgba(0,0,0,0.4), inset 0 1px 0 rgba(0,144,200,0.1) !important;
...
[data-testid="metric-container"]:hover {
    ...
    box-shadow: 0 8px 28px rgba(0,0,0,0.5), 0 0 0 1px rgba(0,144,200,0.2) !important;
}
...
.stTextArea > div > div > textarea:focus {
    ...
    box-shadow: 0 0 0 3px rgba(0,144,200,0.15), 0 2px 8px rgba(0,0,0,0.3) !important;
...
/* Boton normal */
.stButton > button {
    background: linear-gradient(135deg, #0a2e50 0%, #0a3e68 100%) !important;
    color: var(--cream) !important;
...
.stButton > button:hover {
    background: linear-gradient(135deg, #0a4a7a 0%, #0070a0 100%) !important;
    border-color: var(--gold) !important;
    color: var(--gold-pale) !important;
    box-shadow: 0 6px 20px rgba(0,112,160,0.4), inset 0 1px 0 rgba(255,255,255,0.1) !important;
    transform: translateY(-2px) !important;
}
...
.stFormSubmitButton > button {
    background: linear-gradient(135deg, #0a4a80 0%, #0078b0 100%) !important;
    ...
    width: 100% !important;
    box-shadow: 0 4px 16px rgba(0,120,176,0.35) !important;
    transition: all 0.2s !important;
}
.stFormSubmitButton > button:hover {
    background: linear-gradient(135deg, #0068a0 0%, #0090c8 100%) !important;
    box-shadow: 0 8px 24px rgba(0,144,200,0.5) !important;
    transform: translateY(-2px) !important;
}

/* Formulario contenedor */
[data-testid="stForm"] {
    background: linear-gradient(145deg, var(--bg-card) 0%, #0c1e34 100%);
    border: 1px solid var(--border);
    border-radius: 16px;
    padding: 20px 18px;
    box-shadow: 0 8px 32px rgba(0,0,0,0.4), inset 0 1px 0 rgba(0,144,200,0.08);
}
...
.bed-free {
    background: linear-gradient(135deg, #081828 0%, #0a2038 100%);
    border: 1px solid #1a4060;
    color: #50a0d0;
    box-shadow: 0 2px 8px rgba(0,0,0,0.3), inset 0 1px 0 rgba(0,160,200,0.08);
}
.bed-busy {
    background: linear-gradient(135deg, #0a1828 0%, #0c2038 100%);
    border: 1px solid #1e4878;
    color: #60b0e0;
    box-shadow: 0 2px 8px rgba(0,0,0,0.3), inset 0 1px 0 rgba(32,160,220,0.08);
}
...
.log-item:hover { background: rgba(10,24,40,0.9); }

/* Badges de estructuras */
.struct-badge {
    background: linear-gradient(135deg, #081828 0%, #0c2038 100%);
    border: 1px solid var(--border);
    border-radius: 20px;
    padding: 5px 13px;
    font-size: 11px;
...
.triage-card {
    ...
    transition: transform 0.15s, box-shadow 0.15s;
    background: linear-gradient(135deg, #050e18 0%, #081420 100%);
}
.triage-card:hover {
    transform: translateX(3px);
...
.section-card {
    ...
    padding: 18px 16px;
    margin-bottom: 14px;
    box-shadow: 0 4px 20px rgba(0,0,0,0.35), inset 0 1px 0 rgba(0,144,200,0.06);
}
...
<div style='text-align:center; padding: 10px 0 18px 0; animation: fadeUp 0.6s ease;'>
    <div style='font-family: Outfit, sans-serif; font-size:11px; font-weight:600;
                letter-spacing:4px; color:#2a5878; text-transform:uppercase;
                margin-bottom:8px;'>
        Hospital Central &bull; Sistema Digital
    </div>
    <h1 style='font-family: Cormorant Garamond, serif; color:#20aadc;
               font-size:38px; font-weight:700; margin:0; letter-spacing:3px;
               text-shadow: 0 2px 20px rgba(0,144,200,0.3);'>
        Sala de Urgencias
    </h1>
    <div style='font-family: Cormorant Garamond, serif; font-size:16px;
                color:#5a90b8; letter-spacing:2px; margin-top:4px;'>
...
with left:
    ...
    <div style='font-family: Cormorant Garamond, serif; font-size:20px;
                color:#20aadc; font-weight:600; margin-bottom:2px; letter-spacing:1px;'>
        Registro de Paciente
    </div>
    <div style='font-size:10.5px; color:#2a5878; letter-spacing:1px; margin-bottom:12px;'>
    ...
    st.markdown("""
    <div style='font-family: Cormorant Garamond, serif; font-size:20px;
                color:#20aadc; font-weight:600; margin-bottom:10px; letter-spacing:1px;'>
        Acciones Rapidas
    </div>
    """, unsafe_allow_html=True)
    ...
    # Nota de estructuras
    st.markdown("""
    <div style='margin-top:14px; background:linear-gradient(135deg,#081420,#0c1c30);
                border:1px solid #1a3a5c; border-radius:10px; padding:12px 14px;'>
        <div style='font-size:10px; color:#2a5878; letter-spacing:1px; margin-bottom:6px;
                    font-weight:600; text-transform:uppercase;'>Operaciones activas</div>
        <div style='font-size:11.5px; color:#5a90b8; line-height:1.9;'>
            <span style='color:#0090c8;'>enqueue()</span> &rarr; inserta en cola<br>
            <span style='color:#0090c8;'>dequeue()</span> &rarr; saca de la cola<br>
            <span style='color:#0090c8;'>push() / pop()</span> &rarr; pila historial<br>
            <span style='color:#0090c8;'>Array.set()</span> &rarr; asigna cama
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with mid:
    ...
    <div style='font-size:10.5px; color:#2a5878; letter-spacing:1px; margin-bottom:10px;'>
            ...
            with col:
                if bed["status"] == "Libre":
                    st.markdown(
                        ...
                        f"<div style='font-size:9px;color:#3a7898;letter-spacing:1px;'>LIBRE</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                else:
                    ...
                    st.markdown(
                        ...
                        f"<div style='font-size:11px;font-weight:700;'>C{bed['bed']}</div>"
                        f"<div style='font-size:9px;color:#3a6890;'>{p['name'][:8]}</div>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
    ...
    <div style='font-family: Cormorant Garamond, serif; font-size:18px;
                color:#a8d4f0; font-weight:600; margin:10px 0 4px; letter-spacing:1px;'>
        Alta Medica
    </div>
    <div style='font-size:10.5px; color:#2a5878; letter-spacing:1px; margin-bottom:8px;'>
    ...
    if occupied:
        ...
        st.markdown(
            "<div style='background:#081828;border:1px solid #1a3858;border-radius:8px;"
            "padding:10px 14px;font-size:12px;color:#3a7898;text-align:center;'>"
    ...
    if not queue:
        st.markdown(
            "<div style='background:linear-gradient(135deg,#081828,#0c2038);"
            "border:1px solid #1a3858;border-radius:10px;padding:14px;"
            "color:#3a7898;font-size:13px;text-align:center;"
        ...
        for i, p in enumerate(queue):
            ...
            st.markdown(
                ...
                f"{level_name}</span>"
                f"</div>"
                f"<div style='color:#3a6888;font-size:11px;margin-top:3px;'>"
                f"{p['id']} &bull; {p['age']} anios &bull; Espera: {wait_str}"
                f"</div>"
                f"<div style='color:#4a7898;font-size:11.5px;margin-top:3px;"
...
with right:
    ...
    if not events:
        st.markdown(
            "<div style='color:#1a3a5c;font-size:12px;text-align:center;"
            "padding:10px;font-style:italic;'>Sin eventos registrados</div>",
            unsafe_allow_html=True
        )
    else:
        for ev in events[:14]:
            c = event_colors.get(ev["type"], "#2a5878")
            st.markdown(
                f"<div class='log-item'>"
                f"<span style='color:{c};font-weight:700;font-size:10px;"
                f"letter-spacing:1px;'>{ev['type']}</span>"
                f"<span style='color:#1a3a5c;'> &bull; </span>"
                f"<span style='color:#2a5878;font-size:10px;'>{ev['time']}</span>"
                f"<span style='color:#1a3a5c;'> &bull; </span>"
                f"<span style='color:#4a7898;font-size:11px;'>{ev['patient_id']}</span>"
                f"<br><span style='color:#3a6888;font-size:11px;'>{ev['description']}</span>"
                f"</div>",
                unsafe_allow_html=True
            )
        if len(events) > 14:
    ...
    if not alta_list:
        ...
        for p in reversed(alta_list[-7:]):
            ...
            st.markdown(
                f"<div class='alta-item' style='background:linear-gradient(135deg,#0a1828,#0e2038);"
                ...
                f"<span style='color:#2a4a68;font-size:10px;'>{p['id']}</span>"
                f"</div>"
                f"<div style='color:#3a6888;font-size:10.5px;margin-top:2px;'>"
    ...
    if not stack_items:
        ...
        for i, item in enumerate(stack_items[:6]):
            is_top = i == 0
            border_c = "#0090c8" if is_top else "#1a3a5c"
            bg = "linear-gradient(135deg,#0c2038,#081828)" if is_top else "linear-gradient(135deg,#081828,#060e18)"
            label = "TOPE" if is_top else f"[{i+1}]"
            label_c = "#0090c8" if is_top else "#1a3a5c"
            ...
            st.markdown(
                ...
                f"  <span style='color:#a0c8e0;font-weight:600;'>{aname}</span>"
                f"  <span style='color:#2a4a68;font-size:11px;'>{item['patient_id']}</span>"
                f"</div>",
                unsafe_allow_html=True
            )
...
with st.sidebar:
    ...
    <div style='text-align:center; padding:10px 0 16px;'>
        <div style='font-family:Cormorant Garamond,serif; font-size:22px;
                    color:#20aadc; font-weight:600; letter-spacing:2px;'>
            Escala de Triage
        </div>
        <div style='font-size:10px; color:#2a5878; letter-spacing:2px; margin-top:2px;'>
            SISTEMA MANCHESTER
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for lvl, info in TRIAGE_LEVELS.items():
        st.markdown(
            ...
            f"</div>"
            f"<div style='color:#5a90b8;font-size:11.5px;margin-top:3px;'>"
            f"{info['label']}</div>"
            f"</div>",
            unsafe_allow_html=True
        )
    ...
    st.markdown("""
    <div style='font-family:Cormorant Garamond,serif; font-size:20px;
                color:#20aadc; font-weight:600; letter-spacing:1px; margin-bottom:10px;'>
        Estructuras Usadas
    </div>
    """, unsafe_allow_html=True)

    structs = [
        ("Array",           "Camas - slots fijos",     "#0090c8", "get/set O(1)"),
        ("Pila (Stack)",    "Historial y deshacer",    "#20aadc", "push/pop O(1)"),
        ("Cola (Queue)",    "Pacientes en espera",     "#5a9840", "enqueue O(n log n)"),
        ("Lista Enlazada",  "Log de eventos",          "#4a7aaa", "prepend O(1)"),
        ("Lista (list)",    "Pacientes dados de alta", "#6080b8", "append O(1)"),
    ]
    for sname, sdesc, scolor, scomplexity in structs:
        st.markdown(
            f"<div style='background:linear-gradient(135deg,#060e18,#0a1828);"
            ...
            f"<div style='color:#3a6888;font-size:11px;'>{sdesc}</div>"
            f"<div style='color:#1a3a5c;font-size:10px;font-family:monospace;"
    ...
    st.divider()
    st.markdown(
        "<div style='text-align:center;color:#1a3a5c;font-size:10.5px;"
